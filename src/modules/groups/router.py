...
@router.delete("/{group_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Grubu sil (soft)")
async def delete_group(
    ...
    if not group:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Grup bulunamadı.")
    if group.created_by != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Yalnızca grup sahibi silebilir.")
    await services.soft_delete_group(db, group)
